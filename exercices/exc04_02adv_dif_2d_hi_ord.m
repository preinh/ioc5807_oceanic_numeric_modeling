...
Dh=0.25;            % coeficiente de difusao horizontal [m2/s]
qhu=2*dt*Dh/dx2;
qhv=2*dt*Dh/dy2;
concorte=1e-3;     % concentracao de corte para graficos
xgrid=((1:jmax)-1)*dx;
ygrid=((1:kmax)-1)*dy;
...
fant=zeros(kmax,jmax); % linha, coluna invertida.... (our matlab convention)
...
% if umed <0
%     %adveccao u negativo
%     T1=(dt/dx)*umed.*(fatu(2:end-1,3:end)  -fatu(2:end-1,2:end-1)); 
% else
%     %adveccao u positivo
%     T1=(dt/dx)*umed.*(fatu(2:end-1,2:end-1)-fatu(2:end-1,1:end-2)); 
% end
% if vmed <0
%     %adveccao v negativo
%     T2=(dt/dy)*vmed.*(fatu(3:end,  2:end-1)-fatu(2:end-1,2:end-1)); 
% else
%     %adveccao v positivo
%     T2=(dt/dy)*vmed.*(fatu(2:end-1,2:end-1)-fatu(1:end-2,2:end-1)); 
% end
% % difusao em u
% T3=qhu*(fatu(2:end-1,3:end)  -2*fatu(2:end-1,2:end-1)+fatu(2:end-1,1:end-2));
% % difusao em v
% T4=qhv*(fatu(3:end,  2:end-1)-2*fatu(2:end-1,2:end-1)+fatu(1:end-2,2:end-1)); 
